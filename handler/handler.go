...
func (h *Handler) GetAllCountriesHandler(c echo.Context) error {
	var countries []CountrySmall
	err := h.db.Select(&countries, "SELECT Code, Name FROM country")
	if err != nil {
		log.Printf("failed to get countries data\n")
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (h *Handler) GetAllCityHandler(c echo.Context) error {
	...
	if err != nil {
		log.Printf("failed to get cities data\n")
		return c.NoContent(http.StatusInternalServerError)
	}
	log.Printf(cities[len(cities)-1].Name.String)
	return c.JSON(http.StatusOK, cities)
}

func (h *Handler) GetCitiesByCountry(c echo.Context, countryCode string) error {
	var cities []City
	err := h.db.Select(&cities, "SELECT * FROM city WHERE CountryCode = ?", countryCode)
	if err != nil {
		log.Printf("failed to get cities data which cities' CountryCode is %s\n", countryCode)
		return c.NoContent(http.StatusInternalServerError)
	}
